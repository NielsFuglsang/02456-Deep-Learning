...
class Policy(nn.Module):
    ...
    def trpo_loss(self, log_pi, sampled_log_pi, advantage, new_dist, old_dist):
        ratio = torch.exp(log_pi - sampled_log_pi)
...
old_policy = type(policy)(encoder=encoder, feature_dim=feature_dim, num_actions=num_actions)
...
while step < total_steps:
    ...
    for epoch in range(num_epochs):
        ...
        for batch in generator:
            ...
            #print(b_log_prob.size())
            # Get current policy outputs
            old_dist, _ = old_policy(b_obs)
            ...
            loss = policy.trpo_loss(new_log_prob, b_log_prob, b_advantage, new_dist, old_dist)
            loss.backward()
